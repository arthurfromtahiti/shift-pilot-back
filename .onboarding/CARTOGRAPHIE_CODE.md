...
| Routes GET /orders | if-block | 18-26 | Branchement + orchestration filtres (userId, active). Paramètres optionnels fournis par query string, appliqués en cascade. Enrichit chaque commande retournée avec `totalXpf: Math.round(total/100)` (arrondi du montant en centimes). **C'est ici que la logique métier est composée.** |
